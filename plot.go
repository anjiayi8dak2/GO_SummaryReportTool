...
//func generateBarItems(field2Count int) []opts.BarData {
//
...
func runPlot(distanceUnits string, massUnits string, energyUnits string, pollutant string, X1 string, X2 string, Y string, queryResult [][]string) {
	...
	for i := 0; i < len(transposedQueryResult); i++ {
		...
		if transposedQueryResult[i][0] == X1 {
			columnSize := len(transposedQueryResult[0])
			field1 = transposedQueryResult[i] //copy row
			field1 = field1[1:columnSize]     //delete first element
		}
		if transposedQueryResult[i][0] == X2 {
			columnSize := len(transposedQueryResult[0])
			field2 = transposedQueryResult[i]
			field2 = field2[1:columnSize]
		}
		if transposedQueryResult[i][0] == Y {
			columnSize := len(transposedQueryResult[0])
	...
	fmt.Println("inside runPlot valueColumnRow ", valueColumnRow)
	...
	fmt.Println("inside runPlot printing dummy matrix")
	fmt.Println(dummyMatrix)
